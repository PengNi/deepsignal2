"""the feature extraction module of deepsignal2.
output format:
...
def _get_label_raw(fast5_fn, correct_group, correct_subgroup):
    ...
    try:
        event = fast5_data[
            "/Analyses/" + correct_group + "/" + correct_subgroup + "/Events"
        ]
        # print(event)
    except Exception:
        fast5_data.close()
...
def _get_alignment_attrs_of_each_strand(strand_path, h5obj):
    ...
    if sys.version_info[0] >= 3:
        try:
            ...
            alignstrand = (
                str(alignstrand) if type(alignstrand) is not str else alignstrand
            )
...
def _get_readid_from_fast5(h5file):
    ...
    if sys.version_info[0] >= 3:
        try:
            read_id = str(
                h5file["/".join([reads_group, first_read])].attrs["read_id"], "utf-8"
            )
        except TypeError:
...
def _get_alignment_info_from_fast5(
    fast5_path,
    ...
    for fast5_fp in fast5s:
        try:
            ...
            (
                ...
                chrom,
                chrom_start,
            ) = _get_alignment_info_from_fast5(
                fast5_fp, corrected_group, basecall_subgroup
            )
            ...
            for loc_in_read in tsite_locs:
                if num_bases <= loc_in_read < len(genomeseq) - num_bases:
                    ...
                    if alignstrand == "-":
                        ...
                        pos_in_strand = (
                            chromlen - 1 - pos if chromlen is not None else -1
                        )
                    else:
                    ...
                    if (positions is not None) and (
                        key_sep.join([chrom, str(pos), alignstrand]) not in positions
                    ):
                        continue
...
def get_a_batch_features_str(
    ...
    while True:
        ...
        while featurestr_q.qsize() > queue_size_border_f5batch:
            time.sleep(time_wait)
    print(
        "extrac_features process-{} ending, proceed {} fast5s".format(
            os.getpid(), f5_num
        )
    )
...
def _extract_preprocess(
    fast5_dir, is_recursive, motifs, is_dna, reference_path, f5_batch_num, position_file
):
...
def extract_features(
    ...
    for _ in range(nproc):
        p = mp.Process(
            target=get_a_batch_features_str,
            args=(
                fast5s_q,
                featurestr_q,
                errornum_q,
    ...
    p_w = mp.Process(
        target=_write_featurestr, args=(write_fp, featurestr_q, w_batch_num, w_is_dir)
    )
    p_w.daemon = True
    p_w.start()
    ...
    print(
        "%d of %d fast5 files failed..\n"
        "[main]extract_features costs %.1f seconds.."
        % (errornum_sum, len_fast5s, time.time() - start)
    )


def main():
    extraction_parser = argparse.ArgumentParser(
        "extract features from corrected (tombo) fast5s for "
        "training or testing."
