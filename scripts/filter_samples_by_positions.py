...
def filter_one_signal_feature_file(
    sf_fp, positions, wfp, label, chrom_col=1, pos_col=2
):
    wf = open(wfp, "w")
...
def filter_one_signal_feature_file_append(
    sf_fp, positions, wfp, label, chrom_col=1, pos_col=2
):
    wf = open(wfp, "a")
    with open(sf_fp, "r") as rf:
...
def main():
    parser = argparse.ArgumentParser(
        description="extract samples with interested ref_positions "
    ...
    parser.add_argument(
        ...
        default=1,
        required=False,
        help="the col of the chroms in the signal feature file, " "default 1",
    )
    parser.add_argument(
        "--pos_col",
        type=int,
        default=2,
        required=False,
        help="the col of the poses in the signal feature file, " "default 2",
    )
    ...
    if os.path.isdir(sf_fp):
        ...
        for sfile in os.listdir(sf_fp):
            if sfile.find(unique_fid) != -1:
                ...
                filter_one_signal_feature_file_append(
                    read_file, positions, wfp, label, chrom_col, pos_col
                )
