...
def train_1time(train_file, valid_file, valid_lidxs, args):
    ...
    for epoch in range(args.epoch_num):
        ...
        for i, sfeatures in enumerate(train_loader):
            _, kmer, base_means, base_stds, base_signal_lens, base_probs, signals, labels = sfeatures
            if use_cuda:
                kmer = kmer.cuda()
            ...
            # Forward pass
            outputs, tlogits = model(kmer, base_means, base_stds, base_signal_lens, base_probs, signals)
    ...
    for vi, vsfeatures in enumerate(valid_loader):
        _, vkmer, vbase_means, vbase_stds, vbase_signal_lens, vbase_probs, vsignals, vlabels = vsfeatures
        if use_cuda:
            vkmer = vkmer.cuda()
        ...
        voutputs, vlogits = model(vkmer, vbase_means, vbase_stds, vbase_signal_lens, vbase_probs, vsignals)
