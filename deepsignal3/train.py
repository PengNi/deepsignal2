...
def train(args):
    ...
    for epoch in range(args.max_epoch_num):
        ...
        for i, sfeatures in enumerate(train_loader):
            ...
            # Forward pass
            outputs, logits = model(kmer, base_means, base_stds, base_signal_lens, base_probs, signals)
            ...
            if (i + 1) % args.step_interval == 0 or (i + 1) == total_step:
                ...
                with torch.no_grad():
                    ...
                    for vi, vsfeatures in enumerate(valid_loader):
                        _, vkmer, vbase_means, vbase_stds, vbase_signal_lens, vbase_probs, \
